...
Usage: ./Trigger_Extract_WaterLevel.sh [-d FORECAST_DATE] [-c CONFIG_FILE] [-r ROOT_DIR] [-b DAYS_BACK] [-f]
    ...
    -S  Base Date(Timeseries start date) of FLO2D model output in YYYY-MM-DD format. Need to specify in order to use.
    -T  Base Time(Timeseries start time) of FLO2D model output in HH:MM:SS format. Need to specify in oder to use.
...
# NOTE: Need to specify in order to use. Otherwise assume EXTRACTFLO2DWATERLEVEL.py will use RUN_FLO2D.json configs.
